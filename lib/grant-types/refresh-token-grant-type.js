...
function RefreshTokenGrantType(options) {
  ...
  if (!options.model.saveRefreshedToken && !options.model.saveToken) {
    throw new InvalidArgumentError('Invalid argument: model must implement either `saveRefreshedToken()` or `saveToken()`');
}
...
RefreshTokenGrantType.prototype.saveToken = function(fromToken, client) {
  var { user, scope } = fromToken;
  ...
  return Promise.all(fns)
    ...
    .then(function(token) {
      return (this.model.saveRefreshedToken
        ? promisify(this.model.saveRefreshedToken, 3).call(this.model, token, fromToken, client)
        : promisify(this.model.saveToken, 3).call(this.model, token, client, user))
